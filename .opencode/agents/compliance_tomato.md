...
1. **内容红线**：正文是否触及以下任一红线——
   ...
   - 抄袭洗稿
   （亲密内容不在此列，参见下条单独评估）
2. **亲密内容分级审查**（v6 新增·替代原"低俗擦边"二值判定）：
   a. 从 skill 读取 `explicit_content_policy` 和 `core_judgment_rule`
   b. 扫描正文，提取所有亲密互动段落
   c. 逐段判定，按 `level_1_banned`（红线）/ `level_2_allowed`（绿区）/ `level_3_warning`（黄区）归类
   d. 使用 `core_judgment_rule` 核心判据辅助判断
   e. 统计结果：绿N段 / 黄N段 / 红N段
   f. 判定规则：
      - 任一段命中 Level 1（红线）→ 亲密审查不通过，列出违规位置和原因
      - 全部落在 Level 2（绿区）或 Level 3（黄区）→ 亲密审查通过
   g. 对黄区段落给出优化建议（不阻断通过）
3. **番茄节奏适配**：
...
- 向调用方**仅返回一行摘要**，格式固定为：
  ```
  合规:{通过/不通过} | 红线:{通过/不通过} | 节奏:{通过/需优化/不通过} | 亲密:{绿N/黄N/红N}
  ```
