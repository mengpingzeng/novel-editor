...
全书目标总字数 = 盐值 target_total_word_count.calculated_target（若有）
若无此字段 → 从白皮书 §一 估算总字数（取上限）+ 赛道回顾字数范围作为兜底
```
...
- 全书目标总字数：{target_total_word_count} 字（来源：原作估算字数 × 系数）
...
**§5 角色系统输出铁律（vX 新增）**：
- 配角表中"结局"列仅写方向性约束词（如"悲剧性/开放性/HE/隐退"），每条不超过 8 个字
- 配角表末尾追加一行引用标记：
  ```
  > 完整角色命运设计（含逐卷状态变化、性格底层逻辑、命运交叉点）见 versions/{version}/上帝之眼/01-人物命运谱/
  > 结局锚点与最终画面见 上帝之眼/00-全书命运总谱.md §六
  ```
- 结局设计以上帝之眼为准，总纲 §5 的角色"结局"列仅作为方向性约束声明
...
**§6 爽点体系输出铁律（vX 新增·强制比对）**：
- 必须逐条比对 salt.anti_similarity.pleasure_diff 中的声明（新增/删除/变形）
- "新增"声明中的每一个类型 → 必须在总纲 §6 爽点类型清单中出现
- "删除"声明中的每一个类型 → 必须在总纲 §6 文本中显式标注"本仿写移除"
- "变形"声明中的每一个类型 → 必须在对应爽点公式旁标注变形方式
- 比对结果写入 §6 末尾：
  ```
  > 以上爽点类型与 salt.pleasure_diff 声明一致，共 {N} 种（其中新增 {X} 种、删除 {Y} 种、变形 {Z} 种）
  ```
- 任一声明在总纲中无对应 → **报错，禁止输出总纲**
...
**§7 节奏模型输出铁律（vX 新增·终卷约束提取）**：
- 若 salt.volume_rhythm_profile.volume_position_variants 中存在 final_volume：
  → 必须在 §7 末尾追加终卷节奏约束段落
  ```
  ## 7.X 终卷特别约束（来源：salt.volume_position_variants.final_volume）
  - {逐条列出 final_volume.adjustments 中每一条约束，保留原文表述}
  - 结局铺垫章数要求：≥3 章
  - 以上约束将作为终卷事件链设计和卷纲规划的强制输入

  > 注：本段落用于审核可读性和人工交叉校验。上帝之眼从 salt.json 直接读取最终约束，
  > 不依赖本段落的文本解析。本段落仅用于总纲层面的约束留痕。
  ```
- 若 salt 中不存在 final_volume → 省略本段落
- 约束文字必须保持与 salt 原文一致，不可压缩、改写或摘要
