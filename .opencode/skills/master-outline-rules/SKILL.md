...
- 每卷感情互动密度要求：
  (a) ≥1 次暧昧推拉场景
  (b) ≥1 次情感突破场景（关系发生可感知的质变）
  (c) ≥3 次默契互动场景（非宠非护场景中的自然配合）
- 禁止连续 3 章无任何形式的感情互动（暧昧/突破/默契/宠爱——四选一即可）

在"甜宠日常"标签中增加以下约束：
- 互动双向性要求：
  (a) 甜宠载体中"互动型"载体占比 ≥ 30%（观点碰撞后的互相欣赏、沉默陪伴、不经意同步、克制试探）
  (b) 禁止连续 3 个甜宠场景均为单向付出模式
...
**§6 爽点类型扩展模板（v6 新增·感情互动四型）**：

以下四种感情互动爽点类型在生成总纲 §六 时必须包含：

#### P9：暧昧推拉爽（频率 ≈15%，每3章至少1次）

触发公式：
```
{双方处于情感未明朗阶段}
→ {一次互动中产生意料之外的近距离接触/眼神交汇/话语暗含}
→ {至少一方产生可察觉的情绪波动（心跳/闪避/停顿）}
→ {另一方或察觉或未察觉——制造读者"上帝视角"甜感}
→ {收束：互动结束后留下一方独处时的短暂反应}
```

变体池：职场环境中的肢体意外触碰 / 公开场合变相告白被另一方听懂 /
        紧张场景中下意识的保护动作暴露真实心意 / 深夜加班/出差被迫共处一室的边界试探

强度：中


#### P10：情感突破爽（频率 ≈10%，每卷1-2次）

触发公式：
```
{关系处于临界状态——差一步突破但双方或一方在回避}
→ {外部事件或一方主动行为打破当前平衡}
→ {情感阈值被突破——第一次说出在意/第一次主动靠近/第一次袒露脆弱}
→ {另一方反应：震惊→消化→回应（未必正面，但必须有反应）}
→ {关系状态发生可感知的质变}
```

触发类型池：危机触发型 / 嫉妒触发型 / 坦白触发型 / 离别触发型

强度：高


#### P11：默契互动爽（频率 ≈10%，每3章至少1次）

触发公式：
```
{双方已有一定默契基础}
→ {日常/职场场景中无需言语的配合或同步}
→ {一方做事另一方恰好补位 / 一方开口另一方已懂意思}
→ {旁观者或读者感受到"他们之间有一种别人插不进去的东西"}
→ {收束：温暖/安心的余韵}
```

变体池：会议中眼神沟通完成分工 / 对同一件事产生相同判断 /
        一方还没开口另一方已递来需要的东西 / 面对共同敌人时心照不宣的配合

强度：轻中


#### P12：身体对话爽（频率 ≈5%，每卷1-2次）

叙事功能：当语言已不足以承载情感的重量时，身体成为唯一的沟通渠道。
位置：在情感突破（P10）完成之后、或作为 P10 的释放阶段。必须有前面的累积才有此刻的释放。

触发公式：
```
{剧情压力或情感累积达到临界值——语言已无法进一步推进关系}
→ {一方或双方的身体突破了此前的距离边界}
→ {身体感受指向情感意义——抖是因为太久/僵住是因为不敢相信/闭眼是因为不敢看ta的反应}
→ {行为结束后有明确的"关系刻度推进"——不是"他们接吻了"而是"从此以后他们对彼此不一样了"}
→ {余韵收束：一个身体动作承载内心消化过程}
```
