...
def submit_task(prompt):
    ...
    if not task_id:
        raise SystemExit(f"提交任务失败，未获取到 task_id: {json.dumps(result, ensure_ascii=False)}")
    return task_id
...
def poll_task(task_id):
    ...
    while time.time() - start < MAX_WAIT:
        ...
        if resp.status_code != 200:
            raise SystemExit(f"查询任务失败 ({resp.status_code}): {resp.text}")
        result = resp.json()
        ...
        if status == "completed":
            try:
                ...
                raise SystemExit(f"任务完成但未获取到图片URL: {json.dumps(result, ensure_ascii=False)}")
        elif status == "failed":
            error = result.get("error", {})
            raise SystemExit(f"生成失败: {error.get('message', '未知错误')}")
    ...
    raise SystemExit(f"任务超时（{MAX_WAIT}秒），task_id: {task_id}")
...
def main():
    ...
    task_id = submit_task(args.prompt)
